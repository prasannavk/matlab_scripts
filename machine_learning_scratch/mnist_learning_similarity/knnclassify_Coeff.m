function outClass = knnclassify_coeff(sample, TRAIN, group, K, distance,rule)
...
%   $Revision: 1.1.12.6 $  $Date: 2008/06/16 16:32:41 $
...
%load 'GoodCoeff_1.mat';
%load 'coeff_file_2pt5Lakhpts.mat';
%load 'coeff_file_pt4reg.mat';
load 'zero_vs_1_coeff_file_pt1.mat';
switch dist
    case 'euclidean'  % we actually calculate the squared value
        for i = 1:numTest
            D(:,i) = sum(((Train - Test(repmat(i,n,1),:)).^2).*repmat(coeff(1:256)',size(Train,1),1), 2);
        end
    case 'cityblock'
        for i = 1:numTest
